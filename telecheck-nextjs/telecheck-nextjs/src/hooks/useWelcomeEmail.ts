...
export function useWelcomeEmail() {
  ...
  useEffect(() => {
    const sendWelcomeEmail = async () => {
      ...
      try {
        console.log('🎉 Welcome to TeleCheck:', user.email);
        // Welcome emails are sent via Stripe webhooks automatically
        // No manual edge function call needed
      } catch (error) {
        console.error('❌ Welcome process error:', error);
      }
    };

    // Only send welcome email for new subscriptions
    const urlParams = new URLSearchParams(window.location.search);
    if (user && urlParams.get('session_id')) {
      sendWelcomeEmail();
    }
  }, [user]);
}

async function generateWelcomeEmailHTML(email: string): Promise<string> {
  return `
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <div class="container">
        ...
        <div class="content">
            ...
            <p style="color: #e2e8f0; font-size: 16px; margin-bottom: 24px;">
                Thank you for subscribing to TeleCheck.
            </p>
        ...
        <div class="footer">
            ...
            <p class="footer-text" style="margin-top: 16px;">
                © ${new Date().getFullYear()} TeleCheck. Built for Australian healthcare professionals.
            </p>
        </div>
    </div>
</body>
